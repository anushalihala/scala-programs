...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType =
    StructType(
      StructField(columnNames.head, StringType, false) :: columnNames.tail.map(name => StructField(name, DoubleType, false))
    )
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    def colmap(col: String): Int = {
      if (col.startsWith("t01")) 0
      else if (col.startsWith("t03")) 0
      else if (col.startsWith("t11")) 0
      else if (col.startsWith("t1801")) 0
      else if (col.startsWith("t1803")) 0
      else if (col.startsWith("t05")) 1
      else if (col.startsWith("t1805")) 1
      else if (col.charAt(1).isDigit && col.charAt(1) != '5') 2
      else 3
    }

    val col_map = columnNames
      .groupBy(colmap)
      .mapValues(colnames => colnames.map(col(_)))
  ...
  def timeUsageSummaryTyped(timeUsageSummaryDf: DataFrame): Dataset[TimeUsageRow] = {
    ...
    //     .map(row => TimeUsageRow(row.getString(0),row.getString(1), row.getString(2),
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed.groupByKey(row => (row.working,row.sex,row.age))
      ...
      .map(new_row => TimeUsageRow(new_row._1._1, new_row._1._2, new_row._1._3, new_row._2, new_row._3, new_row._4))
