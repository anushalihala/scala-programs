...
object TweetLength {
  ...
  def tweetRemainingCharsCount(tweetText: Signal[String]): Signal[Int] = {
    Signal {
      val curr_tweet = tweetText()
      MaxTweetLength - tweetLength(curr_tweet)
    }
  }

  def colorForRemainingCharsCount(remainingCharsCount: Signal[Int]): Signal[String] = {
    Signal{
      val curr_rem_chars = remainingCharsCount()
      if (curr_rem_chars >= 15) "green"
      else if (curr_rem_chars >= 0) "orange"
      else "red"
    }
  }
