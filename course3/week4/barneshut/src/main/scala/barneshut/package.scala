...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if(size<=minimumSize) Leaf(centerX, centerY, size, bodies.:+(b))
        ...
        val new_fork = Fork(new_nw, new_ne, new_sw, new_se).insert(b)
        var acc = new_fork
        for(b <- bodies){
          acc = acc.insert(b)
        }
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x_coord = min(floor((b.x - boundaries.minX) / sectorSize), sectorPrecision-1)
      val y_coord = min(floor((b.y - boundaries.minY) / sectorSize), sectorPrecision-1)
      val matrix_idx: Int = (y_coord*sectorPrecision + x_coord).toInt
      matrix(matrix_idx) = matrix(matrix_idx).+=(b)
      this
    }
