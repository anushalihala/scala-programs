...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    ...
    val empty_clusters_centres =  means.filterNot(clusters.contains(_))
    empty_clusters_centres.aggregate(clusters)((clusters, centre) => clusters.updated(centre, GenSeq.empty[Point]), _ ++ _)
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (0 until oldMeans.length).par
      .map(i => oldMeans(i).squareDistance(newMeans(i)) <= eta)
      .count(b => !b) == 0
  }

  @tailrec
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val newClusters = classify(points, means)
    val newMeans = update(newClusters, means)
