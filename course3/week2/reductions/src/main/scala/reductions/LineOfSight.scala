...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (idx < input.length){
      ...
      if (angle >= output(idx - 1)) output(idx) = angle
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    def upsweepSeqHelper(idx:Int, maxSoFar: Float): Float ={
      if(idx>=until) maxSoFar
      else {
        val angle = input(idx) / idx
        if (angle >= maxSoFar) upsweepSeqHelper(idx + 1, angle)
        else upsweepSeqHelper(idx + 1, maxSoFar)
      }
    }
    upsweepSeqHelper(from, 0)
  }
