...
import FloatOps._
  ...
  test("Insertion into Fork with 4 empty quadrants") {
    val b = new Body(123f, 17.5f, 27.5f, 0f, 0f)
    ...
    inserted match {
      case Fork(nw,ne,sw,se) => nw match {
        case Leaf(centerX, centerY, size, bodies) => assert({println(nw); true}, s"nw = ${nw}, not a leaf")
        case _ => fail(s"${nw} nw is not a leaf")
      }
      case _ => fail("result of insertion not a fork")
    }
  }
