...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries = {
    val new_boundaries = new Boundaries()
    new_boundaries.minX = min(a.minX, b.minX)
    new_boundaries.maxX = max(a.maxX, b.maxX)
    new_boundaries.minY = min(a.minY, b.minY)
    new_boundaries.maxY = max(a.maxY, b.maxY)
    new_boundaries
  }
